...
def getVelDist(gamma):
    ...
    for i in range(Nvals):
        v_final_max[i] = verne.calcVfinal_full(v_initial_max[i], thetavals[i], depth, sigma_p, m_x,
                                               target)
...
if __name__ == '__main__':
    ...
    Nesc = (scipy.special.erf(vesc / (np.sqrt(2.0) * sigmav)) - np.sqrt(2.0 / np.pi) * (
                vesc / sigmav) * np.exp(-vesc ** 2 / (2.0 * sigmav ** 2)))
    ...
    fname = results_dir + "veldists/f_all_%s_%i_%i_%.1f_%.2f.txt" % (
    loc, args.v_0, args.v_esc, np.log(args.sigma_p), args.m_x)
    ...
    if os.path.exists(fname_avg):
        ...
        try:
            ...
            print(
                f'WARNING:\tOSError while writing {fname_avg} on {time.asctime()}. Taking a nap of 10 minutes and re-trying!')
